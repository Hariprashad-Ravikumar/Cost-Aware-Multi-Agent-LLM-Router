> This documents the original v1 prototype (an n8n-orchestrated pipeline). Its code
> still lives at the repository root (`scripts/`, `n8n/`, `config/`, `results/`,
> `docker-compose.yml`, etc.), only this README has moved here. The actively
> developed system is v2; see the [repository root README](../README.md) and
> [`v2/`](../v2/) for the current, production FastAPI service.
...
All commands above are run from the repository root, since that's where this project's code lives. Set `TEST_MODE=true` before either runner script to process only the first 5 prompts, useful for a fast sanity check before committing to a full 150-prompt run.
